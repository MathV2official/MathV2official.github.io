(function() {
    var MEMORY_KEYS_KEY, MEMORY_MAX_ITEMS, hasProp = {}.hasOwnProperty,
        slice = [].slice,
    ...
    window.I = {
        ...
        sort_keys: function(obj) {
            var append_object, keys;
            keys = [];
        ...
        truncate: function(str, length, tail) {
            ...
            if (str.length > length + tail.length) {
                return "" + str.slice(0, length) + tail
            } else {
                return str
            }
        },
        number_format: function(number, dec, dsep, tsep) {
            var decimals, fnums, parts;
            if (dsep == null) {
                dsep = "."
            }
            if (tsep == null) {
        ...
        format_bytes: function() {
            ...
            return function(bytes) {
                var j, label, len, min, ref;
                for (j = 0, len = thresholds.length; j < len; j++) {
                    ref = thresholds[j], label = ref[0], min = ref[1];
                    if (bytes >= min) {
                        return "" + I.number_format(bytes / min) + label
                    }
                }
                return I.number_format(bytes) + " bytes"
            }
        }(),
        ...
        format_money: function(cents, currency) {
            ...
            if (currency === "JPY") {
                ...
                dollars = I.number_format(cents / 100, 2);
                if (currency === "EUR") {
        ...
        with_recaptcha: function(_this) {
            return function(fn) {
                ...
                if (window.grecaptcha) {
                    ...
                    url = "https://www.google.com/recaptcha/api.js?onload=_itch_recaptcha_loaded&render=explicit";
        ...
        setup_selectize: function(el) {
            return el.find(".selectize_input").each(function(i, input) {
                var $input, descriptions, optgroups, options, params, placeholder, render;
                $input = $(input);
                ...
                if (options = $input.data("options")) {
                    if ($input.hasClass("options_object")) {
        ...
        format_duration: function(duration, num_parts, truncate_seconds) {
            var fn, j, key, len, out, pair, pairs, single, val;
            if (num_parts == null) {
                num_parts = null
            }
        ...
        format_filesize: function(outer, selector) {
            var file_size, j, len, ref, results;
            if (selector == null) {
            ...
            for (j = 0, len = ref.length; j < len; j++) {
                file_size = ref[j];
                file_size = $(file_size);
                results.push(file_size.html(I.format_bytes(parseInt(file_size.html()))))
            }
            return results
        },
        ...
        datetimepicker: function(pickers, opts) {
            var j, len, p, picker, val;
            for (j = 0, len = pickers.length; j < len; j++) {
                p = pickers[j];
                picker = $(p);
                if (val = picker.attr("value")) {
                    picker.val(this.format_date_time(I.parse_timestamp(val)))
                }
            }
        ...
        setup_affiliate_code: function(_this) {
            return function(code, replace) {
                ...
                if (code == null) {
                    code = (ref = window.location.search.match(/\bac=([\w\d]+(?:\-[\.:_\w\d]+)?)/)) != null ? ref[1] : void 0
                }
                if (!code) {
                    return
                }
        ...
        remote_submit: function(_this) {
            return function(form, more_data, opts) {
                var buttons, data;
                if (opts == null) {
                    opts = {}
                }
                ...
                buttons = opts.loading_lock ? form.addClass("loading").find("button, input[type='submit']").prop("disabled", true).addClass("disabled") : void 0;
        ...
        coerce_timestamp: function(ts) {
            var m;
            if (m = typeof ts === "string" && ts.match(/(^\d{4}\-\d{1,2}\-\d{1,2}) (\d{1,2}:\d{1,2}:\d{1,2})$/)) {
                return m[1] + "T" + m[2] + "Z"
            } else {
                return ts
            }
        ...
        parse_timestamp: function(ts) {
            return dayjs(I.coerce_timestamp(ts)).toDate()
        },
        format_timestamp: function() {
            var args, method, ref, t;
            t = arguments[0], method = arguments[1], args = 3 <= arguments.length ? slice.call(arguments, 2) : [];
            if (method == null) {
                method = "fromNow"
            }
            t = I.coerce_timestamp(t);
            switch (method) {
                case "fromNow":
                    return dayjs(t).fromNow();
                case "calendar":
                    return dayjs(t).calendar(null, {
                        sameElse: "MMMM Do YYYY"
                    });
                case "format":
                    return (ref = dayjs(t)).format.apply(ref, args);
                default:
                    throw new Error("unknown method for format_timestamp: " + method)
            }
        },
        store_memory: function(key, value) {
            ...
            if (stored_keys[0] !== key) {
                remaining_keys = function() {
                    var j, len, results;
                    results = [];
                    for (j = 0, len = stored_keys.length; j < len; j++) {
                        k = stored_keys[j];
                        if (key !== k) {
                            results.push(k)
                        }
    ...
    I.InfiniteScroll = function() {
        ...
        function InfiniteScroll(el, opts1) {
        ...
        InfiniteScroll.prototype.setup_loading = function() {
            ...
            if (this.opts.disable_infinite_scroll) {
                return
            }
...
(function() {
    ...
    $.fn.format_timestamps = function(method) {
        var el, fn1, j, len, ref;
        if (method == null) {
            method = "calendar"
        }
        ref = this.find(".date_format");
        fn1 = function(el) {
            var format, full_date, ts;
            format = el.data("format");
            full_date = el.html();
            ts = format ? I.format_timestamp(full_date, "format", format) : I.format_timestamp(full_date, method);
            return el.html(ts).attr("title", full_date + " UTC")
        };
        for (j = 0, len = ref.length; j < len; j++) {
            el = ref[j];
            fn1($(el))
    ...
    $.fn.remote_submit = function(fn, validate_fn, more_data) {
        ...
        return this.on("submit", function(_this) {
            return function(e, callback) {
                ...
                I.remote_submit(form, more_data, {
                    loading_lock: true
                }).done(function(res) {
                    form.data("dirty", false);
                    if (callback != null) {
    ...
    $.fn.track_input = function(selector, name) {
        var inputs, synchronize;
        inputs = this.find(selector);
        synchronize = function(_this) {
            return function() {
                var classes, input, values;
                values = function() {
                    var j, len, results;
                    results = [];
                    for (j = 0, len = inputs.length; j < len; j++) {
                        input = inputs[j];
                        results.push(name + "_" + $(input).attr("value"))
                    }
                    return results
                }();
                _this.removeClass(values.join(" "));
                classes = inputs.serializeArray().map(function(attr) {
                    return name + "_" + attr.value
                });
                return _this.addClass(classes.join(" "))
            }
        }(this);
...
(function() {
            ...
            return $.when(I.add_react(), $.ajax({
                ...
                error: function(_this) {
                    return function(xhr, message, error) {
                        ...
                        errors = (ref = res != null ? res.errors : void 0) != null ? ref : ["Something went wrong. Please try again later and contact support if it persists."];
                        if (R.ErrorLightbox) {
                            return I.Lightbox.open(R.ErrorLightbox({
                                errors: errors
                            }))
                        } else {
                ...
                return function(res, arg) {
                    ...
                    if (props.errors) {
                        if (R.ErrorLightbox) {
                            I.Lightbox.open(R.ErrorLightbox(props))
                        } else {
                            alert(props.errors.join(","));
                            _this.close()
                        }
    ...
    I.ReactLightbox = function(superClass) {
        ...
        function ReactLightbox() {
            var component, el, parent, react_component, rest, wrapper;
            ...
            react_component = ReactDOM.render(component, wrapper[0]);
            el = ReactDOM.findDOMNode(react_component);
...
(function() {
    ...
    I.BuyForm = function(superClass) {
        ...
        BuyForm.prototype.setup_filelist = function() {
            ...
            this.el.find(".file_size_value").html(function() {
                return I.format_bytes(parseInt($(this).html(), 10))
            });
        ...
        BuyForm.prototype.is_valid = function() {
            ...
            if (!BuyForm.__super__.is_valid.call(this)) {
                return
            }
            return true
        };
    ...
    I.BundleBuyForm = function(superClass) {
        ...
        BundleBuyForm.prototype.is_valid = function() {
            ...
            if (!BundleBuyForm.__super__.is_valid.call(this)) {
                return
            }
            return true
        };
...
(function() {
    ...
    I.GameCarousel = function() {
        ...
        GameCarousel.prototype.add_image_loading = function() {
...
(function() {
    var _hex_piece, hex_color, hsl_to_rgb, luma, mix_color, parse_color, parse_color_safe, readable_text_color, relative_luma, rgb_helper, rgb_to_hsl, rgb_to_yuv, scale_luma, sub_color, sub_color2, yuv_to_rgb, slice = [].slice;
    ...
    readable_text_color = function(r, g, b) {
        ...
        y = relative_luma(r, g, b);
        if (y >= .45) {
            return mix_color(hex_color(r, g, b), "#000000", .2)
        } else {
            return "#ffffff"
        }
    };
...
(function() {
    ...
    I.Countdown = function() {
        ...
        Countdown.prototype.update_countdown = function() {
            ...
            dur = dayjs.duration(I.parse_timestamp(this.date) - new Date);
            hidable = true;
            tuples = function() {
                var i, len, results;
                results = [];
    ...
    I.TimestampCountdown = function(superClass) {
        ...
        TimestampCountdown.prototype.update_countdown = function() {
            var duration;
            duration = I.parse_timestamp(this.date) - new Date;
            if (duration < 0) {
                duration = -duration
            }
            return this.el.text(I.format_duration(duration, 1, false))
        };
...
(function() {
    var _grid_id, get_next_id, bind = function(fn, me) {
        return function() {
            return fn.apply(me, arguments)
        }
    };
    ...
    get_next_id = function() {
        return "game_grid_" + _grid_id++
    };
    I.GameGridSizer = function() {
        GameGridSizer.prototype.container_selector = false;
        GameGridSizer.prototype.expected_size = 315;
        GameGridSizer.prototype.aspect_ratio = 315 / 250;
        GameGridSizer.prototype.cell_margin = 10;
        ...
        GameGridSizer.prototype.get_cell_margin = function() {
            if (typeof this.cell_margin === "function") {
                return this.cell_margin()
            } else {
                return this.cell_margin
            }
        };
        GameGridSizer.prototype.get_expected_size = function() {
            if (typeof this.expected_size === "function") {
        ...
        GameGridSizer.prototype.resize_cells = function() {
            var callback, cell_margin, css, expected_width, i, len, new_height, new_width, num_cells, page_width, real_num_cells, real_width, ref, results;
            expected_width = this.get_expected_size();
            cell_margin = this.get_cell_margin();
            real_width = expected_width + cell_margin;
            page_width = this.available_width() + cell_margin;
            ...
            new_width = page_width / real_num_cells - cell_margin;
    ...
    I.GameGrid = function() {
        function GameGrid(el, opts) {
            var copy_fields, field, i, len, size_opts;
            if (opts == null) {
                opts = {}
            }
            ...
            new I.GameCells(this.el);
            if (!this.sizer && this.sizer !== false) {
                copy_fields = ["container_selector", "expected_size", "min_columns", "cell_margin"];
                if (this.el[0] && !this.el[0].id) {
                    this.el[0].id = get_next_id()
                }
                size_opts = {
                    el: this.el,
                    container_selector: "#" + this.el[0].id
                };
                    ...
                    for (i = 0, len = files.length; i < len; i++) {
                        ...
                        if (opts.max_size) {
                            if (file.size > opts.max_size) {
                                errors.push(["You selected an image greater than the max size " + I.format_bytes(opts.max_size) + " (you selected a " + I.format_bytes(file.size) + " file)", file])
                            }
                        }
                    }
                    if (errors.length) {
                            ...
                            return $.when(more_params).then(function(_this) {
                                return function(p) {
                                    return $.ajax({
                                        ...
                                        xhr = I.xhr_upload(file, res).then(function() {
                                            return $.ajax({
                                                url: res.success_url,
                                                type: "post",
                                                xhrFields: {
                ...
                input.on("change", function() {
                    ...
                    for (i = 0, len = ref.length; i < len; i++) {
                        ...
                        if (max_size != null && file.size > max_size) {
                            I.flash(file.name + " is greater than max size of " + I.format_bytes(max_size), "error");
                            continue
                        }
            ...
            success: function(_this) {
                return function(res) {
                    ...
                    if (res.errors) {
                        ...
                        if (R.ErrorLightbox) {
                            I.Lightbox.open(R.ErrorLightbox({
                                errors: res.errors
                            }))
                        } else {
                            alert(err_msg)
...
(function() {
    I.libs.react.done(function() {
        ...
        P("InputRow", {
            ...
            render: function() {
                var inside;
                inside = React.createElement(React.Fragment, {}, div({
                    className: "label",
                    key: "label"
                }, this.props.title, this.props.sub ? span({
                    className: "sub"
                }, " — ", this.props.sub) : void 0), this.props.children);
                if (!this.props.multi) {
                    inside = label({
                        children: inside
                    })
        ...
        P("SimpleSelect", {
            ...
            render: function() {
                ...
                return this.enclose({
                    ...
                    ref: this.select_ref || (this.select_ref = React.createRef()),
                    disabled: this.props.disabled,
                    value: current.value,
                    name: this.props.name,
        ...
        P("FormErrors", {
            ...
            render: function() {
                ...
                }, this.props.title !== false ? p({}, strong({}, this.props.title || this.tt("misc.forms.form_errors"))) : void 0, ul({}, this.props.errors.map(function(_this) {
                    return function(e) {
                        var text;
...
(function() {
    I.libs.react.done(function() {
        ...
        R.Icons.External = create_line_icon({
...
(function() {
    I.libs.react.done(function() {
        ...
        P("ImageUploader", {
            ...
            push_errors: function(res) {
                var err, errors, file, msg;
            ...
            get_progress: function() {
                return Math.min.apply(Math, this.state.active_uploads.map(function(_this) {
                    return function(u) {
                        return u.progress
                    }
                }(this)))
            },
            pick_image: function(pre_callback) {
                this.setState({
                    errors: []
                });
                return I.upload_image({
                    ...
                    max_size: this.props.max_size,
                    test_file: function(_this) {
                        return function(file, d) {
                            return $.when(I.test_image_dimensions(file), I.test_image_format(file)).done(function() {
                                return d.resolve()
                            }).fail(function(err) {
                                return d.reject(err)
                            })
                    ...
                    on_start_upload: function(_this) {
                        return function(file, upload, xhr) {
                            var active_upload;
                            active_upload = {
                                file: file,
                                upload: upload,
                            ...
                            _this.setState(function(state) {
                                return {
                                    active_uploads: state.active_uploads.concat([active_upload]),
                                    uploading: true
                                }
                            });
                            return xhr.progress(function(event, sent, total) {
                                var updated_active_upload;
                                updated_active_upload = Object.assign({}, active_upload, {
                                    progress: sent / total
                                });
                                _this.setState(function(state) {
                                    return {
                                        active_uploads: state.active_uploads.map(function(_this) {
                                            return function(u) {
                                                if (u === active_upload) {
                                                    return updated_active_upload
                                                } else {
                                                    return u
                                                }
                                            }
                                ...
                                return active_upload = updated_active_upload
                            }).done(function(res) {
                                var base;
                                if (res.errors) {
                                    _this.push_errors(res.errors);
                                    return
                                }
                                upload = res.upload;
                                ...
                                return _this.setState(function(state) {
                                    var remaining;
                                    remaining = state.active_uploads.filter(function(_this) {
                                        return function(u) {
                                            return u !== active_upload
                                        }
                                    }(this));
                                    return {
                                        uploading: !!remaining.length,
                    ...
                    return function(res) {
                        return _this.push_errors(res)
                    }
                }(this))
            },
            remove_image: function(upload_idx) {
                var base, to_remove;
                to_remove = this.state.uploads[upload_idx];
                ...
                if (!this.props.multiple) {
                    return typeof(base = this.props).on_upload === "function" ? base.on_upload(null) : void 0
                }
            },
            render_existing_upload: function(upload, idx) {
                ...
                return div({
                    ...
                    onClick: function(_this) {
                        return function(e) {
                            e.preventDefault();
                            return _this.pick_image(function() {
                                return _this.remove_image(idx)
                            })
                        }
                    }(this)
            ...
            render: function() {
                var have_uploads, ref, ref1, ref2, ref3, ref4;
                ...
                return this.enclose({
                ...
                }, this.props.label) : void 0, ((ref1 = this.state.errors) != null ? ref1.length : void 0) ? R.Forms.FormErrors({
                    errors: this.state.errors,
                    title: "Upload failed"
                }) : void 0, !(this.props.multiple || ((ref2 = this.state.uploads) != null ? ref2.length : void 0)) ? input({
                    type: "hidden",
                    name: this.props.name,
                    value: ""
                }) : void 0, this.state.uploading ? div({
                    className: "upload_progress"
                }, this.state.active_uploads.length ? "Uploading " + ((this.get_progress() || 0) * 100).toFixed(2) + "%" : "Uploading...") : void 0, this.props.multiple || !((ref3 = this.state.uploads) != null ? ref3.length : void 0) ? button({
                    id: this.autogen_id,
                    ...
                    onClick: function(_this) {
                        return function(e) {
                            e.preventDefault();
                            return _this.pick_image()
                        }
                    }(this)
                }, this.props.upload_label || (this.props.multiple ? "Upload images" : "Upload image")) : void 0, ((ref4 = this.state.uploads) != null ? ref4.length : void 0) ? div({
...
(function() {
    I.libs.react.done(function() {
        var _rdf, a, br, button, code, current_focus_trap, div, em, fieldset, form, fragment, h1, h2, h3, h4, h5, h6, iframe, img, input, label, legend, li, ol, optgroup, option, p, pre, section, select, span, strong, textarea, types, ul;
        _rdf = ReactDOMFactories;
        div = _rdf.div, span = _rdf.span, a = _rdf.a, br = _rdf.br, p = _rdf.p, ol = _rdf.ol, ul = _rdf.ul, li = _rdf.li, strong = _rdf.strong, em = _rdf.em, img = _rdf.img, form = _rdf.form, label = _rdf.label, input = _rdf.input, textarea = _rdf.textarea, button = _rdf.button, iframe = _rdf.iframe, h1 = _rdf.h1, h2 = _rdf.h2, h3 = _rdf.h3, h4 = _rdf.h4, h5 = _rdf.h5, h6 = _rdf.h6, pre = _rdf.pre, code = _rdf.code, select = _rdf.select, option = _rdf.option, section = _rdf.section, optgroup = _rdf.optgroup, fieldset = _rdf.fieldset, legend = _rdf.legend;
        fragment = React.createElement.bind(null, React.Fragment);
        fragment.type = React.fragment;
        ...
        R.Icons.Close = function(props) {
            ...
            return React.createElement("svg", {
                ...
                version: "1.1",
                viewBox: "0 0 24 24",
                strokeWidth: (ref2 = props.stroke_width) != null ? ref2 : "2",
                width: width,
                height: height,
                strokeLinejoin: "round",
                "aria-hidden": true,
        ...
        R.component("ErrorLightbox", {
            ...
            render: function() {
                return R.Lightbox({
                    className: classNames(this.enclosing_class_name(), "compact")
                }, h2({}, this.props.title || "Your request could not be completed"), R.Forms.FormErrors({
                    title: false,
