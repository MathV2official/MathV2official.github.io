...
function rollForRelic() {
    ...
    possibleRelics.sort(function(a, b) {
        return excavationRewards[a].rarity - excavationRewards[b].rarity;
    });
...
function determineTradeAmounts(paymentType, paymentSubtype, rewardType, rewardSubtype, worldIndex) {
    ...
    if (rewardType == TRADE_TYPE_CHEST || rewardType == TRADE_TYPE_BLUEPRINT ||
        ...
        if (rewardType == TRADE_TYPE_BLUEPRINT) {
            ...
            paymentTimeValue = parseInt(getMoneyBasedTimeValue(paymentTimeValue, valueGain, blueprintPrice, 1 n));
        }
    } else {
...
function getMoneyBasedTimeValue(paymentTimeValue, valueGain, rewardSubtypeValue, valueFraction) {
    ...
    var value3 = bigIntMax(money + getValueOfMinerals(), BigInt(value2)) / 2 n;
...
function determineTimeValueOfTradeType(tradeType, tradeSubtypeIndex = null) {
    ...
    var resultTimeValue = 0 n;
    switch (tradeType) {
        case TRADE_TYPE_ORE:
            resultTimeValue = 3600 n;
            break;
        case TRADE_TYPE_CHEST:
            if (tradeSubtypeIndex == 0) {
                // Basic chest
                resultTimeValue = 600 n;
            } else {
                // Gold chest
                resultTimeValue = 7200 n;
            }
            break;
        case TRADE_TYPE_MONEY:
            resultTimeValue = 3600 n;
            break;
        case TRADE_TYPE_BLUEPRINT:
            resultTimeValue = 14400 n;
            break;
        case TRADE_TYPE_BUFF:
            resultTimeValue = 900 n;
            break;
        case TRADE_TYPE_RELIC:
            if (tradeSubtypeIndex) {
                resultTimeValue = doBigIntDecimalMultiplication(36000 n, Math.pow(excavationRewards[tradeSubtypeIndex].rarity, 2.5));
            }
            break;
        default:
