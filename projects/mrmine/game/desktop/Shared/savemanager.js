...
function runPostLoadingFixes() {
    ...
    if (oldversion < 116) {
        ...
        var purchasedBlueprints = filterBlueprints(blueprintsInCategory, function(blueprint) {
            return blueprint.isFromShop && blueprint.craftedItem.item.canCraft();
        });
...
function loadGame(x) {
    ...
    if (x < RSc) {
        ...
        document.getElementById("TITLED").style.visibility = "hidden";
        document.getElementById("TITLED").style.zIndex = -1;
        document.getElementById("TRANSIENTEFFECTSD").style.zIndex = 1;

        //Append World and UI Layers
        activeLayers["WorldLayer"] = new WorldLayer({
            x: 0,
            y: 0,
            width: mainw,
            height: mainh
        });
        activeLayers["Particles"] = new Particles({
            x: 0,
            y: 0,
            width: mainw,
            height: mainh
        });
        activeLayers["MainUILayer"] = new MainUILayer({
            x: 0,
            y: 0,
            width: mainw,
            height: mainh
        });
        activeLayers["Stage"] = new Stage({
            x: 0,
            y: 0,
            width: mainw,
            height: mainh
        ...
        if (isMobile()) activeLayers["FooterUILayer"] = new FooterUILayer({
            x: 0,
            y: mainh - mobileFooterH,
            width: mobileFooterW,
            height: mobileFooterH
        });
...
function addBackupEntry(key, save, backupTime) {
    ...
    backupEntry.onclick = function() {
        ...
        setTimeout(function() {
            alert(_("Save Import Text Copied To Clipboard"));
        }, 100);
    }
