...
function grantSuperMiner(rarity) {
    ...
    let miner;
    if (!excludeEggs) {
        miner = superMinerManager.getRandomBaseSuperMiner({
            rarity: rarity
        });
    } else {
        miner = superMinerManager.getRandomBaseSuperMiner({
...
function grantMoney(minimumMultiplier, maximumMultiplier, useMineralSpeedMultiplier = true, source) {
    ...
    if (useMineralSpeedMultiplier) {
        moneyAmount = multiplier * doBigIntDecimalMultiplication(valueOfMineralsPerSecond(), STAT.chestMoneyMultiplier()) * 60 n;
    } else {
        moneyAmount = multiplier * doBigIntDecimalMultiplication(valueOfMineralsPerSecond(), STAT.chestMoneyMultiplier() / STAT.minerSpeedMultiplier()) * 60 n;
    }
...
function isCandidateForFirstChestBlueprint() {
    ...
    blueprintsInCategory = filterBlueprints(blueprintsInCategory, function(blueprint) {
        return blueprint.shopSubcategory == _("Discovered");
    });
