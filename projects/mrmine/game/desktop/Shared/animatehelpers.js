...
function drawChromaShift(canvas, id, image, canvasX, canvasY, canvasWidth, canvasHeight) {
    ...
    var channelDrift = {
        r: {
            x: oscillate(numFramesRendered, 9) * -1.33,
            y: oscillate(numFramesRendered, 5) * 1
        },
        g: {
            x: oscillate(numFramesRendered, 3) * 0.5,
            y: oscillate(numFramesRendered, 2) * 0.25
        },
        b: {
            x: oscillate(numFramesRendered, 11) * 1,
            y: oscillate(numFramesRendered, 7) * -1.5
        }
    };
...
function renderChestSliderDots() {
    ...
    for (var i = 0; i < worldClickables.length; i++) {
        if (
            ...
            if (worldClickables[i].type == MINERAL_DEPOSIT_ID) {
                MAIN.drawImage(clickableDot, 0, 0, 6, 10, Math.ceil(mainw * .0544), Math.ceil(mainh * (.317 + (.490 * (Math.min(worldClickables[i].depth + 2, depth) / depth)))) - (Math.ceil(mainh * .007)), Math.floor(mainw * .0048), Math.ceil(mainh * .007));
            } else if (worldClickables[i].type == CAVE_SYSTEM_ID) {
                MAIN.drawImage(caveClickableDot, 0, 0, 10, 10, Math.ceil(mainw * .0544), Math.ceil(mainh * (.317 + (.490 * (Math.min(worldClickables[i].depth + 2, depth) / depth)))) - (Math.ceil(mainh * .007)), Math.floor(mainw * .0048), Math.ceil(mainh * .007));
            } else if (worldClickables[i].type == ORANGE_FISH_ID) {
                MAIN.drawImage(orangeClickableDot, 0, 0, 6, 10, Math.ceil(mainw * .0544), Math.ceil(mainh * (.317 + (.490 * (Math.min(worldClickables[i].depth + 2, depth) / depth)))) - (Math.ceil(mainh * .007)), Math.floor(mainw * .0048), Math.ceil(mainh * .007));
            }
            if (metalDetectorStructure.level > 4) {
                MAIN.globalAlpha = 1;
            }
