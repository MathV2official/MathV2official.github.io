class SettingsWindow extends TabbedPopupWindow {
    ...
    renderStatsPanel() {
        ...
        if (drillState.equippedDrillEquips[0] > -1) {
            context.drawImage(drillState.engine().icon, 0, 0, 50, 50, buydw * .57, buydh * .5, buydw * .07, buydh * .1);
        }
        if (drillState.equippedDrillEquips[1] > -1) {
            context.drawImage(drillState.drill().icon, 0, 0, 50, 50, buydw * .65, buydh * .5, buydw * .07, buydh * .1);
        }
        if (drillState.equippedDrillEquips[2] > -1) {
            context.drawImage(drillState.fan().icon, 0, 0, 50, 50, buydw * .73, buydh * .5, buydw * .07, buydh * .1);
        }
        if (drillState.equippedDrillEquips[3] > -1) {
            context.drawImage(drillState.cargo().icon, 0, 0, 50, 50, buydw * .81, buydh * .5, buydw * .07, buydh * .1);
        }
        ...
        if (mineralAndMoneyLog.length > 0) {
            ...
            for (var i = 60; i > 0; i--) {
                var totalValuePlotPoint = 0 n;
                var mineralValuePlotPoint = 0 n;
