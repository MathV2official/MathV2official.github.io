...
document.getElementById('SIMPLEINPUTFIELD').onmousedown = function(e) {
    e.stopPropagation();
};
...
document.getElementById('LANGUAGESELECTIOND').onmousedown = function(e) {
    e.stopPropagation();
};
...
function bindMouseHandlers(targetElement) {
    ...
    targetElement.addEventListener('wheel', mouseWheelHandler, {
        passive: false
    });
}
...
function writeToMouseDeltaBuffer(deltaX, deltaY) {
    mouseDeltaBuffer[mouseDeltaBufferIndex] = {
        x: deltaX,
        y: deltaY
    };
